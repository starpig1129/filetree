...
export const LandingPage: React.FC<LandingPageProps> = ({ data }) => {
  ...
  return (
    <div className="relative min-h-[calc(100vh-8rem)] flex flex-col items-center justify-center">
      ...
      <div className="w-full max-w-2xl px-2 sm:px-4 relative z-10">

        {/* Title Section */}
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-center mb-2 sm:mb-4 space-y-0.5 sm:space-y-1"
        >
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 md:gap-6">
            <h1 className="text-[clamp(1.5rem,5vw,4rem)] font-bold tracking-tighter animate-stellar-text leading-tight">
              FileNexus
            </h1>
            <Orbit className="w-[clamp(1.2rem,4vw,3rem)] h-[clamp(1.2rem,4vw,3rem)] text-cyan-600 dark:text-quantum-cyan animate-spin-slow opacity-80 dark:opacity-60 hidden sm:block" />
          </div>
          <p className="text-gray-500 dark:text-quantum-cyan/40 tracking-[clamp(0.15em,2vw,0.6em)] font-light uppercase text-[clamp(0.5rem,1vw,0.75rem)] pl-2 sm:pl-4">
        ...
        >
          <div className="neural-border rounded-3xl sm:rounded-4xl p-2.5 sm:p-5 md:p-6 space-y-2 sm:space-y-4 bg-white/95 dark:bg-space-black/80 backdrop-blur-3xl relative overflow-hidden">

            <div className="relative z-10">
              <div className="text-center space-y-0.5 mb-2 sm:mb-4">
                <h2 className="text-[clamp(0.95rem,2vw,1.5rem)] font-bold flex items-center justify-center gap-2 sm:gap-3 leading-tight">
              ...
              <div className="space-y-3 sm:space-y-6">
                <div className="flex p-0.5 bg-gray-100 dark:bg-white/5 rounded-lg sm:rounded-xl border border-gray-200 dark:border-white/5">
                  <button
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <button
                    type="submit"
                    disabled={isSyncing}
                    className="btn-stellar w-full flex items-center justify-center gap-2 sm:gap-3 py-3 sm:py-5 hover:scale-[1.01] active:scale-[0.99] shadow-lg bg-cyan-50 dark:bg-quantum-cyan/10 border-cyan-200 dark:border-quantum-cyan/30 cursor-pointer"
                  >
                    <span className="tracking-[clamp(0.2em,1vw,0.6em)] uppercase font-black text-gray-800 dark:text-white text-[10px] sm:text-sm pl-1 sm:pl-2">
                      {isSyncing ? '資料處理中...' : (uploadType === 'file' ? '開始高速上傳' : '提交資料')}
                    </span>
                    <Zap className={cn("w-3.5 h-3.5 sm:w-5 sm:h-5 text-cyan-600 dark:text-quantum-cyan", isSyncing && "animate-spin")} />
                  </button>
                </form>
              </div>
            </div>
